function wlb_popolate_brainstorm_db 
...
for i=1:ength(list_Dir)
    ...
    for j =1:length(list_file)
        list_file(j).name
        import_to_brainstorm_db(fullfile(root_directory,list_Dir(i).name,list_file(j).name));
    end
end
...
function import_to_brainstorm_db(eegFname)

[p,f,e] = fileparts(eegFname);
...
drug = split{3};
trial = split{4};
condition = strjoin({experiments,drug},'_');

[sSubject, iSubject] = bst_get('Subject', subjects);
if(isempty(sSubject))
    [sSubject, iSubject] = db_add_subject(subjects);
end
[sStudy, iStudy] = bst_get('Study', condition);
if(isempty(sStudy))
    iStudy = db_add_condition(iSubject,condition);
end
