...
for i=1:NR
    modes_white_noise{i}=emdc_fix([],white_noise{i});%calculates the modes of white gaussian noise
end;
...
k=1;
aux=zeros(size(x));
% es_imf = min(size(emdc_fix([],medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter)));
es_imf = min(size(emdc_fix([],medias(end,:),MaxIter,1)));
...
while es_imf>1 %calculates the rest of the modes
    for i=1:NR
        ...
        if tamanio(1)>=k+1
            ...
            noise=Nstd*noise;
            try
%                 [temp,o,it]=emdc(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
                [temp,it]=emdc_fix([],medias(end,:)+std(medias(end,:))*noise,MaxIter,1);
            catch    
                it=0; disp('catch 1 '); disp(num2str(k))
%                 temp=emdc(medias(end,:)+std(medias(end,:))*noise,'MAXMODES',1,'MAXITERATIONS',MaxIter);
                temp=emdc_fix([],medias(end,:)+std(medias(end,:))*noise,MaxIter,1);
            end;
            temp=temp(end,:);
        else
            try
%                 [temp, o, it]=emd(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
                [temp, it]=emdc_fix([],medias(end,:),MaxIter,1);
            catch
%                 temp=emd(medias(end,:),'MAXMODES',1,'MAXITERATIONS',MaxIter);
                temp=emdc_fix([],medias(end,:),MaxIter,1);
